...
# -----> UBUNTU
    if [ -n "$(grep Ubuntu /etc/issue)" -o "$(lsb_release -is 2>/dev/null)" == 'Ubuntu' -o -n "$(grep 'Linux Mint' /etc/issue)" ];then
        ...
        echo "UBUNTU"
        HTTP_DIR="/etc/nginx/"
        HTTP_CONFIG=${HTTP_DIR}"mbilling.conf"
...
#    apt install -y autoconf automake devscripts gawk ntpdate ntp g++ git-core curl sudo xmlstarlet libmyodbc unixodbc-bin apache2 lib>
#    apt install -y php-fpm php php-mcrypt php-dev php-common php-cli php-gd php-pear php-cli php-sqlite3 php-curl php-mbstring unzip >


    echo mysql-server mysql-server/root_password password ${password} | debconf-set-selections
    echo mysql-server mysql-server/root_password_again password ${password} | debconf-set-selections            
    apt install -y curl build-essential
    apt install -y php-fpm php-mysql unzip git php-cli php-pear php-xml php7.4-cli php7.4-json php7.4-opcache php7.4-readline php7.4-xml
    apt install -y fontconfig-config fonts-dejavu-core libfontconfig1 libgd3 libjbig0 libjpeg-turbo8 libjpeg8 libonig5 libtiff5 libwebp6 libxpm4 php-gd php-mbstring php7.4-gd php7.4-mbstring
    apt install -y unixodbc unixodbc-dev libncurses-dev libncurses5-dev uuid-dev libxml2-dev sqlite3 sudo apt-get install libsqlite3-dev
    apt install -y mariadb-server mariadb-client libcurl4-openssl-dev libgnutls-openssl27 ssmtp
    apt purge -y apache2
    apt autoremove -y
    apt install -y nginx
elif [ ${DIST} = "DEBIAN" ]; then
...
wget https://raw.githubusercontent.com/magnussolution/magnusbilling7/source/build/MagnusBilling-current.tar.gz -O /var/www/html/mbilling/MagnusBilling-current.tar.gz
...
sleep 1
cd /usr/src
wget http://www.digip.org/jansson/releases/jansson-2.7.tar.gz -O /usr/src/jansson-2.7.tar.gz
...
wget http://downloads.asterisk.org/pub/telephony/asterisk/asterisk-13-current.tar.gz -O /usr/src/asterisk-13-current.tar.gz
